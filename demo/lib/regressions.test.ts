...
/**
 * The banded audiogram (results screen). Severity bands and speech sounds
 * make a threshold NAMEABLE instead of a dot the reader has to measure
 * against an axis. Both are opt-in, so the clinician's overlay and every
 * other caller keep the plain clinical chart.
 *
 * Scope note: this treatment was tried on the booking and supervision
 * screens too and pulled back — neither had chart-shaped data, so the chart
 * was decoration on the first and an empty box on the second. The audiogram
 * is where it belongs, because the audiogram is already the artifact.
 */
describe("banded audiogram", () => {
  const src = sourceOf("components/charts/audiogram.tsx");
  const resultsSrc = sourceOf("components/screens/patient/results.tsx");

  it("names every severity band", () => {
  ...
  // through its middle and stops reading as a mark.
  it("fills plot marks against a painted band", () => {
    expect(src).toMatch(/fill=\{bands \? "#fff" : "none"\}/);
  });

  // Defaulting these on would silently restyle every existing caller,
  // including the audiologist's clinical overlay.
  it("keeps bands and speech sounds opt-in", () => {
    expect(src).toMatch(/bands = false/);
    expect(src).toMatch(/speech = false/);
  });
  ...
  it("keeps the results screen off slate-400", () => {
    expect(resultsSrc, "results.tsx re-introduced text-slate-400").not.toMatch(/text-slate-400/);
  });
});

/**
 * Two real collisions, found by screenshotting the built screen rather than
 * by reading the code. A band label sits inside the plot area, so it shares
 * space with both the curve and the axis — and moving text away from one
 * collision is how you land in the other.
 */
describe("audiogram band labels collide with nothing", () => {
  const src = sourceOf("components/charts/audiogram.tsx");

  // Bug 1: right-anchored labels were struck through by the curve, because
  // hearing loss slopes down to exactly that corner.
  it("anchors the labels away from where the curve lands", () => {
    const label = /<text x=\{(PAD_L \+ \d+|W - PAD_R - \d+)\} y=\{[^}]+\}\s+textAnchor="(start|end)"/
      .exec(src);
    expect(label, "could not find the band label text element").toBeTruthy();
  ...
  // Bug 2: left-anchoring then put the labels level with the dB axis
  // numbers, reading as one line of text straight across the chart.
  ...
  it("gives the dB axis labels room to render in full", () => {
