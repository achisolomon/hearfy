...
export function Live({go,back}:{go:(s:ScreenId)=>void;back:()=>void}){return <Shell dark><PageHeader title="Connected to your audiologist" subtitle="Dr. Susan Reed is reviewing your test in real time." onBack={back} eyebrow={visitEyebrow("live")}/><div className="relative overflow-hidden rounded-[28px] bg-gradient-to-br from-[#e9f4f1] to-[#bddeea] pt-10 text-center text-brand-navy"><div className="mx-auto grid h-36 w-36 place-items-center rounded-full bg-white text-4xl font-extrabold shadow-card">SR</div><h2 className="mt-5 text-xl font-extrabold">Dr. Susan Reed, Au.D.</h2>{/* Not slate-500: this line sits on the call card's gradient, whose
            darkest end (#BDDEEA) leaves the muted token at 4.22:1. A deeper
            ink keeps it at 5.85:1 across the whole gradient. */}
        <p className="mt-1 text-sm text-[#3f5061]">Licensed Audiologist · Florida</p><div className="mt-8 flex justify-center gap-4 bg-white/70 py-5"><button className="grid h-14 w-14 place-items-center rounded-full bg-white"><Volume2/></button><button className="grid h-14 w-14 place-items-center rounded-full bg-white"><Video/></button><button aria-disabled className="grid h-14 w-14 place-items-center rounded-full bg-red-500/40 text-white cursor-default"><Phone className="rotate-[135deg]"/></button></div></div><p className="mt-5 flex items-center justify-center gap-2 text-sm text-[#8be0d8]"><span className="h-2 w-2 rounded-full bg-[#47d0c4]"/>Secure clinical connection</p><div className="mt-5"><AudiologistStatusLine>Dr. Reed is reviewing your results with you.</AudiologistStatusLine></div></Shell>}
