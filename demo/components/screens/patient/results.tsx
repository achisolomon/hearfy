  ...
  <div className="space-y-4">
    {ears.map(e=><Card key={e.label} className="p-5">
      <div className="mb-2 flex items-baseline justify-between">
        <b className="text-[10px] font-extrabold uppercase tracking-widest text-slate-400">{e.label}</b>
        <span className="text-xs font-bold text-brand-navy">{e.avg} dB HL · {lossBand(e.avg)}</span>
      </div>
      <Audiogram ear={e.ear}/>
    </Card>)}
  </div>
